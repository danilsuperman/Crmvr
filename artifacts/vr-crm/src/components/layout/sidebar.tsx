...
import { BookOpen, Users, Settings, User, Cpu, Monitor, LayoutTemplate, BarChart3, Globe, ChevronDown, Plus, X, Camera, MessageSquare, Brain, Trophy, Heart, Bot, PanelLeftClose, PanelLeftOpen, Smartphone } from "lucide-react";
...
export function Sidebar() {
  ...
  const [collapsed, setCollapsed] = useLocalStorage<boolean>("vrpark_sidebar_collapsed", false);
  ...
  return (
    <>
      <aside className={cn(
        "hidden md:flex border-r border-border bg-sidebar h-full flex-col transition-all duration-300 overflow-hidden",
        collapsed ? "w-16" : "w-64"
      )}>
        ...
        <div className="h-14 flex items-center justify-between px-3 border-b border-border shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            {!collapsed && (
              <span className="font-semibold text-base tracking-tight truncate">VR Park OS</span>
            )}
          </div>
          <button
            onClick={() => setCollapsed(c => !c)}
            className="w-7 h-7 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-colors shrink-0"
        ...
        {/* Park Switcher — only when expanded */}
        {!collapsed && (
          <div className="px-3 py-2 border-b border-border/50 relative" ref={dropdownRef}>
            <button
        ...
        {/* Collapsed: park dot */}
        {collapsed && (
          <div className="px-3 py-2 border-b border-border/50 flex justify-center">
            <div className="w-3 h-3 rounded-full animate-pulse" style={{ backgroundColor: currentPark.color }} />
          </div>
        )}
        ...
        <nav className="flex-1 py-3 flex flex-col gap-0.5 px-2 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className="block relative" title={collapsed ? item.label : undefined}>
                <div className={cn(
                  "flex items-center h-9 rounded-md transition-colors cursor-pointer group hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
          ...
          <Link href="/network" className="block relative" title={collapsed ? "Сеть парков" : undefined}>
            <div className={cn(
              "flex items-center h-9 rounded-md transition-colors cursor-pointer hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
              collapsed ? "justify-center px-2" : "px-3",
              ...
              {!collapsed && <span className="ml-3 font-medium text-sm truncate">Сеть парков</span>}
        ...
        <div className="p-2 border-t border-border shrink-0">
          <Link href="/profile" className="block" title={collapsed ? "Профиль" : undefined}>
            <div className={cn(
              "flex items-center h-10 rounded-md transition-colors cursor-pointer hover:bg-sidebar-accent",
              collapsed ? "justify-center px-2" : "px-3",
